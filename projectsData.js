const PROJECTS_DATA = [
  ...
  },
  {
    ...
    "overview": "The agent is an AI analyst for consumer research data. Food and beverage companies run taste tests and collect thousands of survey answers; this assistant lets staff ask questions like \u201cwhich product was liked most?\u201d or \u201cwhat are people\u2019s main complaints?\u201d in everyday language. It finds the right data on its own, runs the proper statistics, presents results as tables and charts, and stays honest about how confident the numbers are. Safety rails check every database query it writes and keep it from reading another company\u2019s data.\n\n\nJust as important is how the assistant gets improved. Every change follows a strict, documented workflow: first pinpoint where a problem actually comes from, test that diagnosis with a real run, make the smallest possible fix, then re-test with practice questions. The catch \u2014 the grading is done by separate AI reviewers that never wrote the change: one re-derives the correct answer from scratch to check the math, one inspects the output against written style and behavior rules, and one compares how the assistant worked step-by-step against an ideal reference run. The engineer reviews, the owner approves, and only then does a change go live \u2014 so the product improves by evidence, not guesswork.",
    "tech_stack": [
      "LangGraph",
      "FastAPI",
      "PostgreSQL",
    ...
    "features": "Plain-English Q&A over survey data with charts and statistics; every fix goes through a gated workflow graded by independent AI reviewers, with human approval before release",
