const PROJECTS_DATA = [
  ...
  },
  {
    ...
    "extra_images": [],
    "extra_videos": [],
    "description": "An AI assistant that lets non-technical users get answers about consumer taste-test surveys by simply asking in plain English \u2014 plus the disciplined workflow used to improve it without breaking it.",
    "overview": "The agent is an AI analyst for consumer research data. Food and beverage companies run taste tests and collect thousands of survey answers; this assistant lets staff ask questions like \u201cwhich product was liked most?\u201d or \u201cwhat are people\u2019s main complaints?\u201d in everyday language. It finds the right data on its own, runs the proper statistics, presents results as tables and charts, and stays honest about how confident the numbers are. Safety rails check every database query it writes and keep it from reading another company\u2019s data.\n\n\nJust as important is how the assistant gets improved. Every change follows a strict, documented workflow backed by a system for testing, regression checking, and causality analysis built on Claude Code agent skills: three independent AI reviewers \u2014 oracle_agent, trace_compare, and contract_review \u2014 grade every change the engineer who made it could be biased about. oracle_agent independently re-derives the correct answer from scratch to fact-check the numbers; trace_compare judges whether the result and the step-by-step approach match an ideal reference run; contract_review checks the output against written behavior rules and verifies that each fix actually addressed its intended cause. Only after these checks pass does a human review and approve the change for release \u2014 so the product improves by evidence, not guesswork.",
    "tech_stack": [
      "LangGraph",
      "FastAPI",
      "PostgreSQL",
    ...
    "features": "Plain-English Q&A over survey data with charts and statistics; a Claude Code skills workflow (oracle_agent, trace_compare, contract_review) provides testing, regression and causality checks \u2014 every fix graded by independent AI reviewers before a human approves release",
